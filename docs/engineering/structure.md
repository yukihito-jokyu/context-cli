...
- `internal/domain` はCLI、対話UI、YAML、`log/slog`、OSファイルAPIへ依存しない。
...
- YAMLエンコード、XDGパス解決、権限検証、ファイルロック、原子的置換は `internal/infrastructure/` に置く。
...
- 新しい外部技術はinfrastructureまたはcliの境界へ閉じ込め、domainへ漏らさない。
