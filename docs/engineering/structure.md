...
└── internal/                     # 外部非公開のプロジェクト専用モジュール群
    ...
    ├── workspace/                # 開発リポジトリの識別と正規化
    ├── skillcatalog/             # Context Repository内の有効なSkill候補の収集
    ├── distribution/             # 配布計画の策定（Planner）および安全な配布・削除実行（Executor）
    └── distributionmap/          # 配布管理情報（map.yaml）のトランザクション制御とアトミック保存
```

## ディレクトリの責務

- `cmd/context/`: `main` パッケージを置き、`Factory` の作成、Cobraの `RootCmd` の呼び出し、プロセス実行開始（`Execute()`）を担当する。
- `pkg/cmd/`: 各種コマンド（`root`, `init`, `add`, `delete` 等）の定義、フラグ・位置引数のパース、および `Factory` を介したI/Oフローの制御や対話プロンプトを担当する。
...
- `internal/workspace/`: 開発リポジトリ（Workspace）のパス識別や、字句的な正規化を行う。
- `internal/skillcatalog/`: Context Repositoryから、現在選択可能なプロジェクト固有Skillや共通Skillの候補を探索・収集する。
- `internal/distribution/`: 配布計画を検証・策定する `Planner`、および一時退避・ロールバック機構を備えた安全なファイル配布・削除処理を実行する `Executor` を提供する。
- `internal/distributionmap/`: `map.yaml` を用いた配布管理情報のトランザクション制御や、排他ロック制御、アトミックな書き換え保存処理を担当する。
