...
- `internal/cli/`: コマンド解析、対話UI、引数と入出力、終了コード、CLI向けログ表示、およびファイル操作や対話UIなどのI/Oフローの調整（オーケストレーション）を担当する。
- `internal/application/`: 配布可否判定や記録データ構築など、副作用（I/OやUI）を持たない純粋なビジネスルールを定義する。
- `internal/domain/`: 配布物、選択状態、競合、検証などのプロダクト規則と、純粋なドメイン能力のインターフェースを定義する。
- `internal/infrastructure/`: ファイルシステム、YAML、XDG設定パス、ロック、原子的置換、時刻などの具体的なI/O処理を提供する。
...
- `internal/cli` はユーザー入力、対話プロンプト、設定やファイルのI/Oフロー全体を調整・実行し、ビジネスルール判定のために `internal/application` を呼び出す。必要なI/Oポート（インターフェース）は `internal/cli` が自ら（呼び出し側として）定義する。
- `internal/application` は副作用のない純粋なビジネスルール（判定、データ構築など）をカプセル化し、CLI、対話UI、YAML、OSファイルシステムなどのI/Oに一切関与しない（インポートも行わない）。
...
- `internal/infrastructure` はcliが定義するI/Oポートやdomain/applicationの型を実装するが、具象ユースケースやcliのハンドラ自体へは依存しない。
- ログが必要な場合は利用側で必要最小限のインターフェースを定義し、infrastructureまたはcliで `log/slog` に接続する。

## Dependency Direction

- 許可する主要な依存方向は `cli → application → domain` とする。
- `infrastructure` はdomainの型、およびapplicationが定義するデータ構造（DTO）にのみ依存する。
...
- コマンド、フラグ、プロンプト、標準入出力、終了コード、およびI/Oフローの実行制御は `internal/cli/` に置く。
- 副作用を持たないビジネスルール検証、DTOの構築、中断判定は `internal/application/` に置く。
