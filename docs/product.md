...
- 初期版の主要操作である設定、配布、再配布、同期、および削除の受入条件をすべて満たす。
- 選択したプロジェクトと配布物が、定義された優先順位と配置規則どおりに配布される。
- 選択済みの配布物だけを、Context Repositoryの現在の内容へ同期できる。
- 既存データやローカル編集を、ユーザーの許可なく上書き（または削除）する事象を0件にする。
- 配布先で原本や配布定義を個別管理せずに、同じContext Repositoryから繰り返し配布できる。

## 現在の注力事項

初期版として、ローカルのContext Repositoryを設定する。また、カレントディレクトリへの安全な配布、再配布、同期を可能にする。さらに、不要になったSkillを削除できる状態を完成させる。配布対象は、Context Repositoryに原本があるAGENTS.md、CLAUDE.md、共通・リポジトリ固有Skillとする。プロジェクト名を指定しない場合は、Context Repository内のプロジェクトから対話的に選択できるようにする。
...
- 既存データとローカル編集は、明示的な許可なしに変更・削除しない。
...
- `context delete` は不要になったSkillを選択して削除する。
- 配布物の選択は `context add` で行い、削除は `context delete` で行う。
