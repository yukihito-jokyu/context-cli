...
- ST-003: 変更確認で `y` 以外、EOF、入力キャンセル、または入力ストリームの読み取りエラーとなった場合、設定を変更せず、設定変更キャンセルとして判定可能なエラーを返す。
...
- `[MODIFY]` `pkg/cmd/init.go`: 設定更新前に検証処理を呼び出す。既存設定と異なる場合だけ現在値と変更先を表示して確認し、承認後に検証済み絶対パスを保存・表示する。設定変更キャンセルを判定可能なエラーとして定義する。
